...
app.get('/api/get/weatherdata/forecast/', (req, res) => {
    ...
    var request = require('request');
    request(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${APIKEY}`,
        function(error,response, body){
            if(error){
                console.log("Something goes wrong", error);
            }
...
//Test the API = http://localhost:8000/api/get/countrycity?cca2={Enterthe countrycde here}
app.get('/api/get/countrycity/', (req, res) => {   
    const {cca2} = req.query;  
    let owm_country = cca2;
   const sqlGet_Special_City_2 = 'SELECT * from owm_city_list where owm_country LIKE ?' ;
